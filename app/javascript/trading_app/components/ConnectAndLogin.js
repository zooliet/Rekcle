...
class ConnectAndLogin extends React.Component {
  ...
  render() {
    const { connectionInfo, connect, disconnect, setAddress, login } = this.props.kiwoomStore
    ...
    return (
      <div>
        ...
        }
        {
          ...
          <div className='d-flex flex-column align-items-center bg-light jumbotron'>
            <button className="btn btn-success btn-lg p-4" onClick={() => login()}>키움증권 로그인</button>
          </div>
        }        
      </div>
    )
